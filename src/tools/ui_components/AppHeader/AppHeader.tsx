...
export const AppHeader = () => {
    ...
    const { items }: Items = useSelector((state: RootState) => state.cartModule)
    const history = useHistory()

    const getCountItems = (): number | string => {
        const countItems: number | string = items.reduce((acc: number, item: ICartItem) => {
